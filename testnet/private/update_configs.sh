#!/bin/bash
CONFIG_PATH="./network/data/defund/config/config.toml"
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
  ...
  sed -i 's/timeout_prevote =.*/timeout_prevote = "1000ms"/g' $CONFIG_PATH
  sed -i 's/timeout_precommit =.*/timeout_precommit = "1000ms"/g' $CONFIG_PATH
  sed -i 's/timeout_commit =.*/timeout_commit = "1000ms"/g' $CONFIG_PATH
  ...
  printf "\t timeout_prevote = \"1000ms\"\n"
  printf "\t timeout_precommit = \"1000ms\"\n"
  printf "\t timeout_commit = \"1000ms\"\n"
