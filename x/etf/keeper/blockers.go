...
// SendPendingTransfersFromRemote takes all pending transfers from the store
// and sends the IBC transfers for each transfer. These transfers represent
// the assets being brought to defund for redeems. If an error occurs we just
// log and continue to the next iteration as we do not want to stop all transfers for
// one transfer error.
func (k Keeper) SendPendingTransfersFromRemote(ctx sdk.Context) {
	transfers := k.brokerKeeper.GetAllTransfer(ctx)
	for _, transfer := range transfers {
		// get client and then get current height of the counterparty chain
		channel, found := k.channelKeeper.GetChannel(ctx, "transfer", transfer.Channel)
