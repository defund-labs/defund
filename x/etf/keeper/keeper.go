...
func (k Keeper) RedeemShares(ctx sdk.Context, creator string, fund types.Fund, amount sdk.Coin, addressMap types.AddressMap) error {
	...
	for brokerId, msg := range msgs {
		...
		sequence, channel, err := k.SendIBCSend(ctx, msg, fund.Address, broker.ConnectionId)
		if err != nil {
			return err
		}
		...
		k.brokerKeeper.SetRedeem(ctx, redeem)
	}
...
func (k Keeper) CheckHoldings(ctx sdk.Context, holdings []*types.Holding) error {
	...
	for _, holding := range holdings {
		...
		pool, err := k.GetOsmosisPool(ctx, holding.PoolId)
		if err != nil {
			return err
		}
...
func (k Keeper) getOsmosisRoutes(ctx sdk.Context, currentDenom string, needDenom string) (routes []osmosisgammtypes.SwapAmountInRoute, err error) {
	...
	for _, pool := range broker.Pools {
		osmoPool, err := k.GetOsmosisPool(ctx, pool.PoolId)
		if err != nil {
			return routes, err
		}
...
func (k Keeper) CreateRebalanceMsgs(ctx sdk.Context, fund types.Fund) (types.RebalanceMsgs, error) {
	...
	for _, holding := range fund.Holdings {
		...
		if overUnderCompPerc.IsPositive() && !overUnderCompPerc.IsZero() {
			...
			priceInBaseDenom, err := k.CalculateOsmosisSpotPrice(ctx, holding.PoolId, holding.Token, fund.BaseDenom.OnBroker)
			if err != nil {
				return msgs, err
			}
			...
			case "osmosis":
				...
				msg, err = k.CreateOsmosisTrade(ctx, fundBrokerAddress, routes, tokenIn, tokenOut)
				if err != nil {
					return msgs, err
				}
...
func (k Keeper) SendRebalanceTx(ctx sdk.Context, fund types.Fund) error {
	...
	if len(msgs.Osmosis) > 0 {
		...
		channel, sequence, err := k.SendOsmosisTrades(ctx, msgs.Osmosis, fund.Address, broker.ConnectionId)
		if err != nil {
			return err
		}
		// set the rebalance for this broker in store
		rebal := brokertypes.Rebalance{
			Id:     fmt.Sprintf("%s-%d", channel, sequence),
			Fund:   fund.Symbol,
