...
// QueryCallbackCreate clears all queued creates in the create queue and creates + sends them to the creator of shares.
func (k Keeper) QueryCallbackCreate(ctx sdk.Context, query querytypes.InterqueryResult) error {
	creates := k.brokerKeeper.GetAllCreateWithDep(ctx, query.Storeid, "queued")

	// for each queued create, create shares
	for _, create := range creates {
		fund, found := k.GetFund(ctx, create.Fund)
		if !found {
			return sdkerrors.Wrapf(types.ErrFundNotFound, "OnTransferSuccess: fund %s not found", create.Fund)
		}
		...
		numETFShares, err := k.GetAmountETFSharesForToken(ctx, fund, *create.Token)
		if err != nil {
			return err
		}
		...
		creatorAcc, err := sdk.AccAddressFromBech32(create.Sender)
		if err != nil {
			return err
		}
		...
		// log success and remove create from store
		ctx.Logger().Debug(fmt.Sprintf("create shares transfer %s was completed successfully", create.Id))
		k.brokerKeeper.RemoveCreate(ctx, create.Id)
	}
...
func (k Keeper) OnTransferSuccess(ctx sdk.Context, packet channeltypes.Packet, ack channeltypes.Acknowledgement) error {
	transfer, found := k.brokerKeeper.GetCreate(ctx, fmt.Sprintf("%s-%d", packet.SourceChannel, packet.Sequence))
	...
	k.brokerKeeper.RemoveCreate(ctx, transfer.Id)
	return nil
}
