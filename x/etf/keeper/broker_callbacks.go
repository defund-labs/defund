...
func (k Keeper) OnRedeemSuccess(ctx sdk.Context, packet channeltypes.Packet, redeem brokertypes.Redeem) error {
	...
	fund, found := k.GetFund(ctx, redeem.Fund)
	if !found {
		return sdkerrors.Wrapf(etftypes.ErrFundNotFound, "fund %s not found", redeem.Fund)
	}
	fund.Shares.Amount = fund.Shares.Amount.Sub(redeem.Amount.Amount)
	k.SetFund(ctx, fund)

	// lets clear up the redeem from the store
	k.brokerKeeper.RemoveRedeem(ctx, redeem.Id)
...
func (k Keeper) OnRedeemFailure(ctx sdk.Context, packet channeltypes.Packet, redeem brokertypes.Redeem) error {
...
func (k Keeper) OnRebalanceSuccess(ctx sdk.Context, rebalance brokertypes.Rebalance, symbol string) error {
	fund, found := k.GetFund(ctx, symbol)
	if !found {
		return sdkerrors.Wrapf(etftypes.ErrFundNotFound, "fund %s not found", symbol)
	}
	fund.LastRebalanceHeight = int64(ctx.BlockHeight())
	fund.Rebalancing = false
	k.SetFund(ctx, fund)
	// Remove the rebalance from store. Clean up store
	k.brokerKeeper.RemoveRebalance(ctx, rebalance.Id)
	return nil
}
...
func (k Keeper) OnRebalanceFailure(ctx sdk.Context, rebalance brokertypes.Rebalance, symbol string) error {
	fund, found := k.GetFund(ctx, symbol)
	if !found {
		return sdkerrors.Wrapf(etftypes.ErrFundNotFound, "fund %s not found", symbol)
	}
