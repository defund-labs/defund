...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			minPrice, err := sdk.LegacyNewDecFromStr(args[2])
			if err != nil {
				return fmt.Errorf("invalid min price: %w", err)
			}

			maxPrice, err := sdk.LegacyNewDecFromStr(args[3])
			if err != nil {
				return fmt.Errorf("invalid max price: %w", err)
			}

			initialPrice, err := sdk.LegacyNewDecFromStr(args[4])
			if err != nil {
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			offerCoin, err := sdktypes.ParseCoinNormalized(args[2])
			if err != nil {
				return fmt.Errorf("invalid offer coin: %w", err)
			}

			demandCoinDenom := args[3]
			if err := sdktypes.ValidateDenom(demandCoinDenom); err != nil {
				return fmt.Errorf("invalid demand coin denom: %w", err)
			}

			price, err := sdk.LegacyNewDecFromStr(args[4])
			if err != nil {
