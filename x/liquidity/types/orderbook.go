...
func OrderBookBasePrice(ov amm.OrderView, tickPrec int) (math.LegacyDec, bool) {
...
func MakeOrderBookPairResponse(pairId uint64, ov *amm.OrderBookView, lowestPrice, highestPrice math.LegacyDec, tickPrec int, configs ...OrderBookConfig) OrderBookPairResponse {
	...
	var smallestPriceUnit math.LegacyDec
...
func PrintOrderBookResponse(ob OrderBookResponse, basePrice math.LegacyDec) {
	fmt.Println("+------------------------------------------------------------------------+")
...
func FitPriceToTickGap(price, gap math.LegacyDec, down bool) math.LegacyDec {
	b := price.BigInt()
	b.Quo(b, gap.BigInt()).Mul(b, gap.BigInt())
	tick := math.LegacyNewDecFromBigIntWithPrec(b, math.LegacyPrecision)
