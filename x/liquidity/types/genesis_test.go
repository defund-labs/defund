...
func TestGenesisState_Validate(t *testing.T) {
	...
	depositReq := types.DepositRequest{
		...
		PoolId:         1,
		MsgHeight:      1,
		Depositor:      sdktypes.AccAddress(crypto.AddressHash([]byte("depositor"))).String(),
		DepositCoins:   utils.ParseCoins("1000000denom1,1000000denom2"),
		MintedPoolCoin: sdktypes.NewInt64Coin("pool1", 0),
	...
	withdrawReq := types.WithdrawRequest{
		...
		PoolId:     1,
		MsgHeight:  1,
		Withdrawer: sdktypes.AccAddress(crypto.AddressHash([]byte("withdrawer"))).String(),
		PoolCoin:   sdktypes.NewInt64Coin("pool1", 1000000),
		Status:     types.RequestStatusNotExecuted,
	}
	order := types.Order{
		Id:                 1,
		PairId:             1,
		MsgHeight:          1,
		Orderer:            sdktypes.AccAddress(crypto.AddressHash([]byte("orderer"))).String(),
		Direction:          types.OrderDirectionSell,
		OfferCoin:          sdktypes.NewInt64Coin("denom1", 1000000),
		RemainingOfferCoin: sdktypes.NewInt64Coin("denom1", 500000),
		ReceivedCoin:       sdktypes.NewInt64Coin("denom2", 500000),
