...
func TestMsgCreatePool(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			func(msg *types.MsgCreatePool) {
				msg.DepositCoins = sdktypes.Coins{utils.ParseCoin("0denom1"), utils.ParseCoin("1000000denom2")}
			},
		...
		},
		{
			"invalid deposit coins",
			func(msg *types.MsgCreatePool) {
				msg.DepositCoins = sdktypes.Coins{utils.ParseCoin("1000000denom1"), utils.ParseCoin("0denom2")}
			},
...
func TestMsgCreateRangedPool(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			"too small min price",
			func(msg *types.MsgCreateRangedPool) {
				msg.MinPrice = sdk.LegacyNewDecWithPrec(1, 16)
			},
		...
		},
		{
			"too large max price",
			func(msg *types.MsgCreateRangedPool) {
				msg.MaxPrice = sdk.NewIntWithDecimal(1, 25).ToLegacyDec()
			},
...
func TestMsgCancelAllOrders(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			msg := types.NewMsgCancelAllOrders(sdktypes.AccAddress(crypto.AddressHash([]byte("orderer"))), []uint64{1, 2, 3})
			tc.malleate(msg)
