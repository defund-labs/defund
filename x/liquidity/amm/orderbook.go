...
func (ob *OrderBook) LowestPrice() (sdk.LegacyDec, bool) {
	lowestBuyPrice, _, foundBuy := ob.buys.lowestPrice()
	lowestSellPrice, _, foundSell := ob.sells.lowestPrice()
	switch {
	case foundBuy && foundSell:
		return sdk.LegacyMinDec(lowestBuyPrice, lowestSellPrice), true
	case foundBuy:
...
func (ob *OrderBook) stringRepresentation(prices []sdk.LegacyDec) string {
	if len(prices) == 0 {
		return "<nil>"
	}
...
func (ob *OrderBook) String() string {
	priceSet := map[string]sdk.LegacyDec{}
	for _, tick := range append(ob.buys.ticks, ob.sells.ticks...) {
		priceSet[tick.price.String()] = tick.price
	}
	prices := make([]sdk.LegacyDec, 0, len(priceSet))
...
func (ticks *orderBookTicks) findPrice(price sdk.LegacyDec) (i int, exact bool) {
...
func (ticks *orderBookTicks) ordersAt(price sdk.LegacyDec) []Order {
...
func (ticks *orderBookTicks) highestPrice() (sdk.LegacyDec, int, bool) {
	if len(ticks.ticks) == 0 {
		return sdk.LegacyDec{}, 0, false
	}
	if ticks.priceIncreasing {
...
func (ticks *orderBookTicks) lowestPrice() (sdk.LegacyDec, int, bool) {
	if len(ticks.ticks) == 0 {
		return sdk.LegacyDec{}, 0, false
	}
	if ticks.priceIncreasing {
