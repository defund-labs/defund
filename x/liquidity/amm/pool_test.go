...
func TestBasicPool(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		require.True(sdk.LegacyDecEq(t, pool.Price(), highest))
		lowest, found := pool.LowestSellPrice()
		require.True(t, found)
		require.True(sdk.LegacyDecEq(t, pool.Price(), lowest))
	}
}
...
func TestBasicPool_Price(t *testing.T) {
	for _, tc := range []struct {
		...
		p      sdk.LegacyDec // expected pool price
	}{
		{
			name: "normal pool",
			ps:   10000,
		...
		},
		{
			...
			rx:   200,
			ry:   300,
			p:    sdk.LegacyMustNewDecFromStr("0.666666666666666667"),
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			pool := amm.NewBasicPool(sdk.NewInt(tc.rx), sdk.NewInt(tc.ry), sdk.NewInt(tc.ps))
			require.True(sdk.LegacyDecEq(t, tc.p, pool.Price()))
		})
	}
...
func TestBasicPool_Withdraw(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			ps:      10000,
			pc:      1000,
			feeRate: sdk.LegacyMustNewDecFromStr("0.003"),
			x:       199,
			y:       9,
		},
		{
...
func TestValidateRangedPoolParams(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			"too low min price",
			sdk.LegacyNewDecWithPrec(1, 16), utils.ParseDec("2.0"),
			utils.ParseDec("1.0"),
			"min price must not be lower than 0.000000000000001000",
		},
		{
			"too high max price",
			utils.ParseDec("0.5"), sdk.NewIntWithDecimal(1, 25).ToLegacyDec(),
			utils.ParseDec("1.0"),
...
func TestCreateRangedPool(t *testing.T) {
	intApproxEq := func(exp, got sdk.Int) (*testing.T, bool, string, string, string) {
		...
		if c && !exp.IsZero() {
			c = exp.ToLegacyDec().Sub(got.ToLegacyDec()).Abs().Quo(exp.ToLegacyDec()).LTE(sdk.LegacyNewDecWithPrec(1, 3))
		}
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			sdk.LegacyNewDecWithPrec(1, 15), utils.ParseDec("2.0"),
			utils.ParseDec("1.0"),
			"",
		...
		},
		{
			"large max price",
			sdk.NewInt(1_000000000000000000), sdk.NewInt(1_000000000000000000),
			utils.ParseDec("1.0"), sdk.NewIntWithDecimal(1, 20).ToLegacyDec(),
			utils.ParseDec("2.0"),
			"",
...
func TestRangedPool_Deposit(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			var reserveRatio sdk.LegacyDec
			switch {
			case tc.rx.IsZero():
			...
			require.True(t, utils.DecApproxEqual(reserveRatio.Add(sdk.LegacyOneDec()), transXPrime.Quo(transX)))
			require.True(t, utils.DecApproxEqual(reserveRatio.Add(sdk.LegacyOneDec()), transYPrime.Quo(transY)))
		})
	}
}
...
func TestRangedPool_Withdraw(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			pool := amm.NewRangedPool(tc.rx, tc.ry, tc.ps, tc.minPrice, tc.maxPrice)
			x, y := amm.Withdraw(tc.rx, tc.ry, tc.ps, tc.pc, sdk.LegacyZeroDec())
...
func TestRangedPool_SwapPriceOutOfRange(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		p := utils.RandomDec(r, sdk.LegacyNewDecWithPrec(1, 5), minPrice.Mul(utils.ParseDec("0.99")))
...
func TestBasicPoolOrders(t *testing.T) {
	...
	lowestPrice := poolPrice.Mul(sdk.LegacyNewDecWithPrec(9, 1))
	highestPrice := poolPrice.Mul(sdk.LegacyNewDecWithPrec(11, 1))
