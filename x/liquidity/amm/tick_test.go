...
func TestPriceToTick(t *testing.T) {
	for _, tc := range []struct {
		...
		require.True(sdk.LegacyDecEq(t, tc.expected, amm.PriceToDownTick(tc.price, 3)))
	}
}
...
func TestTick(t *testing.T) {
	for _, tc := range []struct {
		...
		{135000, 3, sdk.LegacyNewDec(1)},
		{135001, 3, utils.ParseDec("1.001")},
	} {
		t.Run("", func(t *testing.T) {
			res := amm.TickFromIndex(tc.i, tc.prec)
			require.True(sdk.LegacyDecEq(t, tc.expected, res))
...
func TestUpTick(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			require.True(sdk.LegacyDecEq(t, tc.expected, amm.UpTick(tc.price, tc.prec)))
		})
	}
}
...
func TestDownTick(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			require.True(sdk.LegacyDecEq(t, tc.expected, amm.DownTick(tc.price, tc.prec)))
		})
	}
}
...
func TestHighestTick(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			require.True(sdk.LegacyDecEq(t, sdk.LegacyNewDecFromBigInt(i), tick))
		})
	}
}
...
func TestLowestTick(t *testing.T) {
	for _, tc := range []struct {
		...
		{0, sdk.LegacyNewDecWithPrec(1, 18)},
		{3, sdk.LegacyNewDecWithPrec(1, 15)},
	} {
		t.Run("", func(t *testing.T) {
			require.True(sdk.LegacyDecEq(t, tc.expected, amm.LowestTick(tc.prec)))
		})
	}
}
...
func TestPriceToUpTick(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			require.True(sdk.LegacyDecEq(t, tc.expected, amm.PriceToUpTick(tc.price, tc.prec)))
		})
	}
}
...
func TestRoundPrice(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			require.True(sdk.LegacyDecEq(t, tc.expected, amm.RoundPrice(tc.price, tc.prec)))
		})
	}
}
...
func TestTickGap(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			require.True(sdk.LegacyDecEq(t, tc.expected, amm.TickGap(tc.price, tc.tickPrec)))
		})
	}
}
