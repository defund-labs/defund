...
func TestOrderBook(t *testing.T) {
	...
	require.True(sdk.LegacyDecEq(t, utils.ParseDec("10.01"), highest))
	lowest, found := ob.LowestPrice()
	require.True(t, found)
	require.True(sdk.LegacyDecEq(t, utils.ParseDec("9.996"), lowest))
}
...
func TestOrderBook_HighestPriceLowestPrice(t *testing.T) {
	for _, tc := range []struct {
		ob           *amm.OrderBook
		found        bool
		highestPrice sdk.LegacyDec
		lowestPrice  sdk.LegacyDec
	}{
		{
			amm.NewOrderBook(),
			false, sdk.LegacyDec{}, sdk.LegacyDec{},
		},
		{
			amm.NewOrderBook(
		...
		t.Run("", func(t *testing.T) {
			...
			if tc.found {
				require.True(sdk.LegacyDecEq(t, tc.highestPrice, highestPrice))
				require.True(sdk.LegacyDecEq(t, tc.lowestPrice, lowestPrice))
			}
		})
	}
}
