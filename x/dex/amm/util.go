...
var (
	zeroInt = sdk.ZeroInt()
	oneDec  = sdk.LegacyOneDec()
	fourDec = sdk.LegacyNewDec(4)
)

// OfferCoinAmount returns the minimum offer coin amount for
// given order direction, price and order amount.
func OfferCoinAmount(dir OrderDirection, price sdk.LegacyDec, amt sdk.Int) sdk.Int {
	switch dir {
	case Buy:
...
func MatchableAmount(order Order, price sdk.LegacyDec) (matchableAmt sdk.Int) {
...
func TotalMatchableAmount(orders []Order, price sdk.LegacyDec) (amt sdk.Int) {
	amt = sdk.ZeroInt()
	for _, order := range orders {
...
func inv(x sdk.LegacyDec) (r sdk.LegacyDec) {
	r = oneDec.Quo(x)
	return
}
...
func poolOrderPriceGapRatio(poolPrice, currentPrice sdk.LegacyDec) (r sdk.LegacyDec) {
	if poolPrice.IsZero() {
		poolPrice = sdk.LegacyNewDecWithPrec(1, sdk.LegacyPrecision) // lowest possible sdk.LegacyDec
	}
