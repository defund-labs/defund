...
type Pool interface {
	...
	HighestBuyPrice() (sdk.LegacyDec, bool)
	LowestSellPrice() (sdk.LegacyDec, bool)
	BuyAmountOver(price sdk.LegacyDec, inclusive bool) sdk.Int
	SellAmountUnder(price sdk.LegacyDec, inclusive bool) sdk.Int
	BuyAmountTo(price sdk.LegacyDec) sdk.Int
	SellAmountTo(price sdk.LegacyDec) sdk.Int
...
func (pool *BasicPool) Price() sdk.LegacyDec {
...
func (pool *BasicPool) HighestBuyPrice() (price sdk.LegacyDec, found bool) {
...
func (pool *BasicPool) LowestSellPrice() (price sdk.LegacyDec, found bool) {
...
// amt = (X - P*Y)/P
func (pool *BasicPool) BuyAmountOver(price sdk.LegacyDec, _ bool) (amt sdk.Int) {
	origPrice := price
...
func (pool *BasicPool) SellAmountUnder(price sdk.LegacyDec, _ bool) (amt sdk.Int) {
...
func (pool *BasicPool) BuyAmountTo(price sdk.LegacyDec) (amt sdk.Int) {
	origPrice := price
	if price.LT(MinPoolPrice) {
		price = MinPoolPrice
	}
...
// price of the order book.
func (pool *BasicPool) SellAmountTo(price sdk.LegacyDec) (amt sdk.Int) {
	if price.GT(MaxPoolPrice) {
		price = MaxPoolPrice
	}
	if price.LTE(pool.Price()) {
...
type RangedPool struct {
	rx, ry             sdk.Int
	ps                 sdk.Int
	minPrice, maxPrice sdk.LegacyDec
	transX, transY     sdk.LegacyDec
	xComp, yComp       sdk.LegacyDec
}

// NewRangedPool returns a new RangedPool.
func NewRangedPool(rx, ry, ps sdk.Int, minPrice, maxPrice sdk.LegacyDec) *RangedPool {
...
func CreateRangedPool(x, y sdk.Int, minPrice, maxPrice, initialPrice sdk.LegacyDec) (pool *RangedPool, err error) {
...
func ValidateRangedPoolParams(minPrice, maxPrice, initialPrice sdk.LegacyDec) error {
...
func (pool *RangedPool) Translation() (transX, transY sdk.LegacyDec) {
	return pool.transX, pool.transY
}

func (pool *RangedPool) MinPrice() sdk.LegacyDec {
	return pool.minPrice
}

func (pool *RangedPool) MaxPrice() sdk.LegacyDec {
	return pool.maxPrice
}

// Price returns the pool price.
func (pool *RangedPool) Price() sdk.LegacyDec {
	if pool.rx.IsZero() && pool.ry.IsZero() {
		panic("pool price is not defined for a depleted pool")
	}
...
// HighestBuyPrice returns the highest buy price of the pool.
func (pool *RangedPool) HighestBuyPrice() (price sdk.LegacyDec, found bool) {
	// The highest buy price is actually a bit lower than pool price,
	// but it's not important for our matching logic.
	return pool.Price(), true
}
...
// LowestSellPrice returns the lowest sell price of the pool.
func (pool *RangedPool) LowestSellPrice() (price sdk.LegacyDec, found bool) {
	// The lowest sell price is actually a bit higher than the pool price,
	// but it's not important for our matching logic.
	return pool.Price(), true
}
...
// BuyAmountOver returns the amount of buy orders for price greater than
// or equal to given price.
func (pool *RangedPool) BuyAmountOver(price sdk.LegacyDec, _ bool) (amt sdk.Int) {
	origPrice := price
...
// or equal to given price.
func (pool *RangedPool) SellAmountUnder(price sdk.LegacyDec, _ bool) (amt sdk.Int) {
...
// where BuyAmountTo is used when the pool price is higher than the highest
// price of the order book.
func (pool *RangedPool) BuyAmountTo(price sdk.LegacyDec) (amt sdk.Int) {
	origPrice := price
	if price.LT(pool.minPrice) {
		price = pool.minPrice
	}
...
// where SellAmountTo is used when the pool price is lower than the lowest
// price of the order book.
func (pool *RangedPool) SellAmountTo(price sdk.LegacyDec) (amt sdk.Int) {
	if price.GT(pool.maxPrice) {
		price = pool.maxPrice
	}
	if price.LTE(pool.Price()) {
...
func Withdraw(rx, ry, ps, pc sdk.Int, feeRate sdk.LegacyDec) (x, y sdk.Int) {
	if pc.Equal(ps) {
	...
	utils.SafeMath(func() {
		...
		multiplier := sdk.LegacyOneDec().Sub(feeRate)                                      // 1 - feeRate
...
func DeriveTranslation(rx, ry sdk.Int, minPrice, maxPrice sdk.LegacyDec) (transX, transY sdk.LegacyDec) {
	sqrt := utils.DecApproxSqrt
...
func PoolOrders(pool Pool, orderer Orderer, lowestPrice, highestPrice sdk.LegacyDec, tickPrec int) []Order {
	return append(
...
func PoolBuyOrders(pool Pool, orderer Orderer, lowestPrice, highestPrice sdk.LegacyDec, tickPrec int) (orders []Order) {
	defer func() {
		if r := recover(); r != nil {
			orders = nil
		}
	...
	tmpPool := pool.Clone()
	placeOrder := func(price sdk.LegacyDec, amt sdk.Int, derive bool) {
	...
	tick := PriceToDownTick(sdk.LegacyMinDec(highestPrice, tmpPool.Price()), tickPrec)
...
func PoolSellOrders(pool Pool, orderer Orderer, lowestPrice, highestPrice sdk.LegacyDec, tickPrec int) (orders []Order) {
	defer func() {
		if r := recover(); r != nil {
			orders = nil
		}
	...
	tick := PriceToUpTick(sdk.LegacyMaxDec(lowestPrice, tmpPool.Price()), tickPrec)
