...
func (s *KeeperTestSuite) TestDefaultGenesis() {
	...
	got := s.app.DexKeeper.ExportGenesis(s.ctx)
...
func (s *KeeperTestSuite) TestImportExportGenesis() {
	...
	s.app.DexKeeper.InitGenesis(s.ctx, genState2)
	genState3 := s.app.DexKeeper.ExportGenesis(s.ctx)

	s.Require().Equal(*genState, *genState3)

	depositReq2, found := s.app.DexKeeper.GetDepositRequest(s.ctx, depositReq.PoolId, depositReq.Id)
	s.Require().True(found)
	s.Require().Equal(depositReq, depositReq2)
	withdrawReq2, found := s.app.DexKeeper.GetWithdrawRequest(s.ctx, withdrawReq.PoolId, withdrawReq.Id)
	s.Require().True(found)
	s.Require().Equal(withdrawReq, withdrawReq2)
	order2, found := s.app.DexKeeper.GetOrder(s.ctx, order.PairId, order.Id)
	s.Require().True(found)
...
func (s *KeeperTestSuite) TestIndexesAfterImport() {
	...
	// Check pair indexes.
	pair, found := s.app.DexKeeper.GetPairByDenoms(s.ctx, "denom1", "denom2")
	s.Require().True(found)
	s.Require().Equal(pair1.Id, pair.Id)

	resp1, err := s.app.DexKeeper.Pairs(sdk.WrapSDKContext(s.ctx), &types.QueryPairsRequest{
	...
	resp2, err := s.app.DexKeeper.Pairs(sdk.WrapSDKContext(s.ctx), &types.QueryPairsRequest{
	...
	// Check pool indexes.
	pools := s.app.DexKeeper.GetPoolsByPair(s.ctx, pair2.Id)
	s.Require().Len(pools, 1)
	s.Require().Equal(pool2.Id, pools[0].Id)

	pool, found := s.app.DexKeeper.GetPoolByReserveAddress(s.ctx, pool1.GetReserveAddressAcc())
	s.Require().True(found)
	s.Require().Equal(pool1.Id, pool.Id)

	// Check deposit request indexes.
	depositReqs := s.app.DexKeeper.GetDepositRequestsByDepositor(s.ctx, s.addr(4))
	s.Require().Len(depositReqs, 1)
	s.Require().Equal(depositReq1.Id, depositReqs[0].Id)

	depositReqs = s.app.DexKeeper.GetDepositRequestsByDepositor(s.ctx, s.addr(5))
	s.Require().Len(depositReqs, 1)
	s.Require().Equal(depositReq2.Id, depositReqs[0].Id)

	// Check withdraw request indexes
	withdrawReqs := s.app.DexKeeper.GetWithdrawRequestsByWithdrawer(s.ctx, s.addr(4))
	s.Require().Len(withdrawReqs, 1)
	s.Require().Equal(withdrawReq1.Id, withdrawReqs[0].Id)

	withdrawReqs = s.app.DexKeeper.GetWithdrawRequestsByWithdrawer(s.ctx, s.addr(5))
	s.Require().Len(withdrawReqs, 1)
	s.Require().Equal(withdrawReq2.Id, withdrawReqs[0].Id)

	// Check order indexes
	orders := s.app.DexKeeper.GetOrdersByOrderer(s.ctx, s.addr(6))
	s.Require().Len(orders, 1)
	s.Require().Equal(order1.Id, orders[0].Id)

	orders = s.app.DexKeeper.GetOrdersByOrderer(s.ctx, s.addr(7))
	s.Require().Len(orders, 1)
