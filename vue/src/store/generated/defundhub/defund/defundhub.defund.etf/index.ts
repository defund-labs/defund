...
export default {
	...
	actions: {
		...
		async QueryFundAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params: {...key}, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.nextKey!=null) {
					let next_values=(await queryClient.queryFundAll({...query, 'pagination.key':(<any> value).pagination.nextKey})).data
		...
		async QueryFundPrice({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params: {...key}, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.nextKey!=null) {
					let next_values=(await queryClient.queryFundPrice({...query, 'pagination.key':(<any> value).pagination.nextKey})).data
					value = mergeResults(value, next_values);
				}
	...
	gas: "200000" }, memo})
				...
				if (e == MissingWalletError) {
					throw new SpVuexError('TxClient:MsgInvest:Init', 'Could not initialize signing client. Wallet is required.')
				}else{
