...
export const Fund = {
  encode(message: Fund, writer: Writer = Writer.create()): Writer {
    ...
    if (message.broker !== "") {
      writer.uint32(58).string(message.broker);
    }
    for (const v of message.holdings) {
      Holding.encode(v!, writer.uint32(66).fork()).ldelim();
    }
    if (message.rebalance !== 0) {
      writer.uint32(72).int64(message.rebalance);
    }
    if (message.baseDenom !== "") {
      writer.uint32(82).string(message.baseDenom);
    }
    if (message.connectionId !== "") {
      writer.uint32(90).string(message.connectionId);
    }
    if (message.creator !== "") {
